...
@section('content')
    ...
    <div class="admin-container">
        <div style="background: #FFFFFF; border-radius: 16px; padding: 32px; box-shadow: 0 4px 16px rgba(0,0,0,0.1); max-width: 700px;">
            <h1 style="color: #1C6EA4; margin-bottom: 24px;">Edit Subject</h1>
            ...
            <form method="POST" action="{{ route('admin.subjects.update', $subject) }}">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        ...
                        <input type="text" name="code" required value="{{ old('code', $subject->subject_code) }}" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                        @error('code')
                    ...
                    <div>
                        ...
                        <input type="number" name="units" required min="1" value="{{ old('units', $subject->units) }}" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                        @error('units')
                            <p style="color: #EF4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <input type="text" name="name" required value="{{ old('name', $subject->subject_name) }}" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                    @error('name')
                        <p style="color: #EF4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div>
                        ...
                        <select name="course" required style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                            ...
                            @foreach($courses as $course)
                                <option value="{{ $course }}" {{ old('course', $subject->course) == $course ? 'selected' : '' }}>{{ $course }}</option>
                            @endforeach
                            <option value="other" {{ old('course', $subject->course) == 'other' ? 'selected' : '' }}>Other (type below)</option>
                        </select>
                        @error('course')
                            <p style="color: #EF4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</p>
                        @enderror
                        <div style="margin-top:8px;">
                            <input type="text" name="custom_course" value="{{ old('custom_course', $subject->course) }}" placeholder="Custom course/program" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                        </div>
                    </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        ...
                        <input type="text" name="semester" value="{{ old('semester', $subject->semester) }}" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                    </div>

                    <div>
                        <label style="display: block; margin-bottom: 6px; color: #374151; font-weight: 500;">Hours Per Week</label>
                        <input type="number" name="hours_per_week" min="1" value="{{ old('hours_per_week', $subject->hours_per_week) }}" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        ...
                        <input type="text" name="schedule" value="{{ old('schedule', $subject->schedule) }}" placeholder="e.g., MWF" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                        @error('schedule')
                            <p style="color: #EF4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="time" value="{{ old('time', $subject->time) }}" placeholder="e.g., 8:00 AM - 9:30 AM" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                        @error('time')
                            <p style="color: #EF4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div style="margin-bottom: 24px;">
                    ...
                    <textarea name="description" rows="3" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">{{ old('description', $subject->description) }}</textarea>
                </div>
                ...
                <div style="display: flex; gap: 12px;">
                    ...
                    <button type="submit" style="padding: 10px 20px; background: #1C6EA4; color: white; border: none; border-radius: 6px; cursor: pointer;">Update Subject</button>
                </div>
            </form>
        </div>
    </div>
